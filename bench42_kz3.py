...
for tag, url in (("run120A", SEARCH), ("run120B", SEARCH), ("run120C", SEARCH),
                 ("landing-control", LANDING)):
    codes, ttfbs = run_seq(url)
    out.append(summary(tag, codes, ttfbs))
open("/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/bench42_out.txt", "w").write("\n".join(out) + "\n")
