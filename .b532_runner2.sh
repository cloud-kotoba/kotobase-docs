...
OUT=/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/.b532_raw2.txt
: > "$OUT"
run_set() {
  local ep="$1" tag="$2"
  for i in {1..23}; do
    curl -o /dev/null -s -w "%{http_code} %{time_starttransfer}\n" "https://${ep}" >> "$OUT.tag.$tag" 2>/dev/null
  done
}
rm -f "$OUT".tag.* 2>/dev/null
run_set "search.kotobase.net/search?q=test" A
run_set "search.kotobase.net/search?q=test" B
run_set "search.kotobase.net/search?q=test" C
run_set "kotobase.net/signup" CTRL
