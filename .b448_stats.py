...
def nearest_rank_p50(vals):
    s = sorted(vals)
    # nearest-rank p50 = ceiling(0.50*N)-th value (1-indexed)
...
def stats(path):
    ...
    return {
        ...
        'min': s[0],
        'max': s[-1],
        'mean': statistics.mean(s) if n else None,
        'p95': s[max(0, min(n-1, int(0.95*n)-1))],
    }

for label, path in [('run448A', sys.argv[1] if len(sys.argv)>1 else '.b448_run448A.ttfb'),
                    ('run448B', '.b448_run448B.ttfb'),
                    ('run448C', '.b448_run448C.ttfb'),
                    ('landing', '.b448_landing.ttfb')]:
    r = stats(path)
    if r is None:
