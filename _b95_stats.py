...
f = "_b95_run231_out.txt"
groups = {"A": [], "B": [], "C": [], "control": []}
for line in open(f):
    line = line.strip()
    if not line:
        continue
    m = re.match(r"run231([ABC])\s+(\d+)\s+([\d.]+)", line)
    if m:
...
for k in ["A","B","C","control"]:
    stats(groups[k], f"run231{k}" if k != "control" else "control(signup)")

allc = [v for k in ["A","B","C"] for v in groups[k] if v >= 0.5]
allv = [v for k in ["A","B","C"] for v in groups[k]]
print(f"TOTAL 18時台 search 60試行: cold={len(allc)}/60 ({len(allc)/60*100:.1f}%)")
