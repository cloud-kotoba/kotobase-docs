#!/usr/bin/env python3
# insert rank iter-log entry after '## Iteration log'
...
for ln in new_lines:
    if ln.strip() == "## Iteration log":
        nc += 1
if nc !=  ̃1:
    raise SystemExit("ERR after-insert header count !=1")
with open(path, "w", encoding="utf-8") as f:
    f.write(out
with open("/tmp/b390_status.txt", "w", encoding="utf-8") as g:
    g.write("hdr_idx=%d hc=%d first=%s lines=%d\n" % (hdr_idx, hc, first, len(new_lines))