#!/bin/bash
# K-Z3 cosientist run169A-C (search) + landing control, same method as prior runs:
# n=20 per run, separate curl connections, record TTFB per request.
OUT=/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/cos58_run169_out.txt
: > "$OUT"
date >> "$OUT"
uptime >> "$OUT"
for run in A B C; do
  echo "=== run169$run search ===" >> "$OUT"
  for i in $(seq 1 20); do
    ttfb=$(curl -s -o /dev/null -w '%{time_starttransfer} %{http_code}' "https://search.kotobase.net/search?q=test")
    echo "169$run $i $ttfb" >> "$OUT"
  done
done
