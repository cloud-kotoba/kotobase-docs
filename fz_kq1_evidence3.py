...
# evidence block ends at the last non-empty line before the next row (blank line separates)
last = end - 1
while len(lines[last].strip()) == 0:
    last -= 1
assert lines[last].rstrip("\n").endswith("委ねる"), repr(lines[last][-40:])
lines[last] = lines[last].rstrip("\n") + evidence + "\n"

open(path, "w", encoding="utf-8").write("".join(lines))
print("appended to K-Q1 evidence block line", last + 1)
