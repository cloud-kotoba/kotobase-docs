...
try:
    ...
    assert "run125A cold(>=0.5s) 1/20" in src, "A"
    marker = "NEXT: 委ねる (rank 判断 — 9時台 n 積み増し継続、または K-Q1 engine 内訳の cosientist 実装指定)。\n"
    assert src.count(marker) == 1, "B count=%d" % src.count(marker)
    entry = (
    ...
    src2 = src.replace(marker, marker + entry, 1)
    assert src2 != src, "C"
    io.open(path, "w", encoding="utf-8").write(src2)
    buf.write("appended OK\n")
except Exception:
    buf.write(traceback.format_exc())
io.open("/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/bench45_log_status.txt", "w").write(buf.getvalue())
