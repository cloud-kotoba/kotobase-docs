// bench49 (第49回) — K-Q1 deploy 後再判別プローブ
// PR #3 (c3c508f, x-kotobase-kv-stats header) の merge (7dc6249) 後 production deploy
// 判別を ephemeral EOA で行う (transact 401 でも header 判別自体は query 200 で可能)。
...
try {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    results.push({
      ...
      readAuditCid: q.response.headers.get("x-kotobase-read-audit-cid"),
    });
  }
  const kvStats = results.map((r) => r.kvStats);
  console.log(JSON.stringify({
    purpose: "bench49 deploy re-probe (x-kotobase-kv-stats header presence x3)",
