...
for k,n,ncold,p50,mx,cv,n200,ntot in out:
    print("RUN%s n=%d cold=%d/%d p50=%.4fs max=%.4fs http200=%d/%d cold_vals:[%s]" % (k,n,ncold,n,p50,mx,n200,ntot,", ".join("%.4f"%v for v in cv)))
...
print("SEARCH_TOTAL cold=%d/%d (~%.1f%%) http200=%d/%d" % (tot_cold,tot_n,100.0*tot_cold/tot_n,tot_200,tot_n))
land = [o for o in out if o[0]=="landing"][0]
print("LANDING cold=%d/%d p50=%.4fs max=%.4fs http200=%d/%d" % (land[2],land[1],land[3],land[4],land[6],land[7]))