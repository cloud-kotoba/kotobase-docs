#!/usr/bin/env python3
# bench K-Z3 run360 stats: cold>=0.5s TTFB, nearest-rank p50, per-file max.
...
def parse(fn):
    ...
    with open(fn) as f:
        for line in f:
            p=line.split()
            if len(p)>=2:
                codes.append(int(p[0])); rows.append(float(p[1]))
    return rows, codes
...
for fn in [".b360_360A.txt",".b360_360B.txt",".b360_360C.txt",".b360_land.txt"]:
    report(fn)