#!/bin/bash
# K-Z3 深夜 5時台 run113A-C (n=20 x3 + landing control)
URL_S="https://search.kotobase.net/search?q=test"
URL_L="https://kotobase.net/"
OUT="/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/kz3_run113_out.txt"
: > "$OUT"
echo "start $(date '+%F %T %z')" >> "$OUT"
uptime >> "$OUT"
for r in A B C; do
  echo "=== run113$r $(date '+%T') ===" >> "$OUT"
