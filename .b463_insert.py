...
# load ev / ilog text, scrub zero-width and stray markers
def scrub(b):
    s = b.decode('utf-8')
...
ev = scrub(open('.b463_ev.txt', 'rb').read()).rstrip(b'\n')
ilog = scrub(open('.b463_ilog.txt', 'rb').read()).rstrip(b'\n')

# --- offsets from base ---
# K-Z3 row start and its terminating newline (row end)
marker = b'| K-Z3 | worker |'
row_start = data.find(marker)
row_nl = data.find(b'\n', row_start)
print('row_start', row_start, 'row_nl', row_nl, file=sys.stderr)

# Iteration log header (line '## Iteration log\n') -> insert newest-first right after header newline
hdr = b'## Iteration log\n'
hi = data.find(hdr)
