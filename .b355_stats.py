...
if __name__=='__main__':
    ...
    # cold detail positions per run
    for c in ['A','B','C']:
        r=parse(base+'355'+c+'.txt')
        coldidx=[i+1 for i,(code,t) in enumerate(r) if code=='200' and t>=0.5]
        vals=[(i+1,round(t,4)) for i,(code,t) in enumerate(r) if code=='200' and t>=0.5]
        print(f"355{c} cold positions/values:", vals)