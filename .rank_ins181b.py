...
entry = ("- 2026-09-08: rank 第181回。04:36 JST tick。HEAD bdffc26 = rank 第180回 (04:05) = "
         ...
         "は falsify/bench 実施待ち in-flight (帯水準確定・機構判断は追加 n 待ちのまま)。"
...
new_content = content[:idx] + header + entry + rest

with io.open(path, "w", encoding="utf-8") as f:
    f.write(new_content)

# verify exactly one header
cnt = new_content.count(header)
print("header_count=%d" % cnt)
print("entry_first_20=%s" % new_content[idx+len(header):idx+len(header)+20])