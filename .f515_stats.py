#!/usr/bin/env python3
# K-Z3 stats for falsify run515 (nearest-rank p50, cold>=0.5s), ASCII-safe output
import os
...
res = []
res.append(stats(os.path.join(BASE, ".f515_run515A.ttfb"), "run515A"))
res.append(stats(os.path.join(BASE, ".f515_run515B.ttfb"), "run515B"))
res.append(stats(os.path.join(BASE, ".f515_run515C.ttfb"), "run515C"))
res.append(stats(os.path.join(BASE, ".f515_landing.ttfb"), "control(signup)"))

outtext = "\n".join(res)
with open(os.path.join(BASE, ".f515_stats.txt"), "w") as f:
    f.write(outtext + "\n")
print(outtext)