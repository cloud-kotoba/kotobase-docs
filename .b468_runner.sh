#!/bin/bash
# K-Z3 production HTTP measurement runner (bench run468). 3 x n=20 search + n=20 landing control.
# Separate curl connections, TTFB via time_starttransfer. Matches run467 method exactly.
OUT="$1"
...
t1=$(now)
echo "BIG468 elapsed_sec $t0 $t1" > "${OUT}_t0.txt"
