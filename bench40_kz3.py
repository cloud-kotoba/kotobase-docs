...
for tag, url in (("run115A", SEARCH), ("run115B", SEARCH), ("run115C", SEARCH),
                 ("landing-control", LANDING)):
    codes, ttfbs = run_seq(url)
    out.append(summary(tag, codes, ttfbs))
open("/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/bench40_out.txt", "w").write("\n".join(out) + "\n")
