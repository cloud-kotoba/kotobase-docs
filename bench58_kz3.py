...
out.append("host load: %s" % subprocess.run(["uptime"], capture_output=True, text=True).stdout.strip())
for tag, url in [(t, SEARCH) for t in RUNS] + [("landing-control", LANDING)]:
    codes, ttfbs = run_seq(url)
    out.append(summary(tag, codes, ttfbs))
open("/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/bench58_out.txt", "w").write("\n".join(out) + "\n")
