...
# evidence row: line index 281 (0-based), starts '| K-Z3 |' and contains 'bench 2026-09-13'... choose the K-Z3 row whose line is NOT followed by a continuation line
cand = [i for i, l in enumerate(lines) if l.startswith("| K-Z3 |")]
# the one with evidence = ends with a long evidence tail; pick the one where previous next line does NOT start with ' bench'
kz3_idx = None
for i in cand:
    nxt = lines[i+1] if i+1 < len(lines) else ""
    if not nxt.startswith(" bench") and not nxt.startswith(" falsify"):
        kz3_idx = i
assert kz3_idx is not None
i = kz3_idx
assert lines[i].endswith("|") or True
tail = lines[i][-60:]
assert "|" in tail, tail
