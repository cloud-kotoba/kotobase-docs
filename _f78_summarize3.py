...
codes = {}
times = {}
for line in open('_f78_run201c_out.txt'):
    line = line.strip()
    m = re.match(r'(\S+) (\d{3}) ([\d.]+)', line)
    if not m:
        continue
    name, code, t = m.group(1), m.group(2), float(m.group(3))
    codes[code] = codes.get(code, 0) + 1
    if line.startswith('run201'):
        r = line[6]
        (warm if t < 0.5 else cold)[r].append(t)
        times.setdefault(name, []).append((code, t))
...
# positions of colds within each run
for r in 'ABC':
    cs = [i+1 for i, (c, t) in enumerate(times['run201'+r]) if t >= 0.5]
    if cs:
        out[r]['cold_positions'] = cs
