// bench63 fallback — K-Z3 3時台 n 積み増し (rank 第63回 NEXT フォールバック)
...
const SEARCH = "https://search.kotobase.net/search?q=bench63kz3";
...
const result = { purpose: "bench63 K-Z3 3時台 n 積み増し (rank 第63回 fallback)", observedAt: new Date().toISOString(), runs: [] };
for (let r = 0; r < 3; r += 1) {
  const runs = [];
