...
OUTF = "/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/.cs489_stats_out.txt"
THRESH = 0.5
...
for k in ["A", "B", "C", "landing"]:
    ...
    p95 = nrank_pct(ttfb, 95)
    mx = max(ttfb) if ttfb else 0.0
    line = "RUN%s n=%d cold=%d/%d p50=%.4fs p95=%.4fs max=%.4fs http200=%d/%d cold_vals:[%s]" % (
        k, n, len(cold), n, p50, p95, mx, n200, len(codes),
