// bench64 — K-Z3 3時台 2 セット目 n 積み増し (rank 第63回 fallback 継続, 本 tick 独自分)
// 同測定法: /search?q= n=20 × 3 run + landing control n=20, 別接続 curl, production 実測 (gate 外)
// run180A 多発 (9/20) の即時非再現確認に利得。secret 不含。
...
const SEARCH = "https://search.kotobase.net/search?q=bench64kz3";
...
const result = { purpose: "bench64 K-Z3 3時台 2 セット目 n 積み増し (run181A-C)", observedAt: new Date().toISOString(), runs: [] };
for (let r = 0; r < 3; r += 1) {
  const runs = [];
