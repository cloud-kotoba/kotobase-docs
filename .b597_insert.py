#!/usr/bin/env python3
# insert K-Z3 23-tai evidence into query-cosientist.md (single physical L279 row) + iteration log line
import re, sys, unicodedata
DOC = "/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/query-cosientist.md"
EV = (" falsify 2026-09-13 (第249回, run597A-C K-Z3 23時台 n 積み増し, 同測定法 n=20 x3 + landing control, 別接続 curl, Tokyo, 23:41-23:44 JST, 全 80/80 200): cold(>=0.5s) 2/60 (~3.3%) - run597A cold 1/20 (523.0ms 境界付近単発) p50 44.1ms / run597B cold 1/20 (518.0ms 境界付近単発) p50 45.3ms / run597C cold 0/20 p50 45.6ms, landing control (kotobase.net/, 同時刻, n=20, 全 200) cold 1/20 (926.1ms) p50 62.9ms - control にも単発が出たため本窓は帯特有性を分離できず (host load1 8.54 低下傾向)。23時台通算は run596 分 (cold 2/60 ~3.3%) と同水準。status 判定は rank に委ねる (rank 専門)。secret 不含 (curl + python3 stats のみ)")
...
# 1) find K-Z3 row (single physical line, starts with | K-Z3 |)
idxs = [i for i,l in enumerate(lines) if l.startswith("| K-Z3 |")]
print("KZ3 rows:", idxs)
assert len(idxs) == 1, "expect single K-Z3 row"
i = idxs[0]
assert lines[i].rstrip().endswith("|"), "row must end with |"
lines[i] = lines[i].rstrip() + EV

# 2) insert iteration log line after '## Iteration log'
