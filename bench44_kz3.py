...
for tag, url in (("run125A", SEARCH), ("run125B", SEARCH),
                 ("run125C", SEARCH), ("landing-control", LANDING)):
    codes, ttfbs = run_seq(url)
    out.append(summary(tag, codes, ttfbs))
open("/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/bench44_out.txt", "w").write("\n".join(out) + "\n")
print("\n".join(out))
