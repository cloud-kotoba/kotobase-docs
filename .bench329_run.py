#!/usr/bin/env python3
"""K-Z3 8時台 run329A-C + landing control. Same measurement method as claim contract:
...
def run_curl(url, timeout=65):
    ...
    http_code, ttfb = parts[0], parts[1]
    try:
        return (int(http_code), float(ttfb), elapsed, "")
    except ValueError:
...
def report(name, items):
    ...
    return {
        ...
        "cold": len(cold),
        "cold_vals": [round(it[1],4) for it in cold],
        "cold_idxs": [it[5] for it in cold],
...
def measure(url, label, n=20):
    ...
    for i in range(1, n + 1):
        ...
        items.append((code, ttfb, elapsed, err, None, i))
        time.sleep(0.8)
...
for tag, url in [("searchA", search_url), ("searchB", search_url), ("searchC", search_url)]:
...
out = {"results": results, "uptime": load,
       "started": time.strftime("%Y-%m-%d %H:%M:%S JST")}
with open(".bench329_out.json", "w") as f:
