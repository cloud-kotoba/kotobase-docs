...
try {
  ...
  // write 1 datom (transact は 401 の場合はそのまま記録して継続)
  const txEdn = `[{ :db/id "${runId}" :bench/run-id "${runId}" :bench/value "kvstats" }]`;
  ...
  console.log(JSON.stringify({
    purpose: "bench49 deploy probe (x-kotobase-kv-stats header presence, with transact)",
    observedAt: new Date().toISOString(),
    writeStatus: write.response.status,
    writeData: write.data,
    queryStatus: firstQuery.response.status,
    queryData: firstQuery.data,
    markerReadBack: JSON.stringify(firstQuery.data).includes("kvstats"),
