...
def scrub(t):
    ...
    for c in t:
        ...
        if cat not in ("Mn", "Cf", "Cc"):
            o.append(c)
    return "".join(o)
ev = scrub(io.open(".b193_run437_ev.txt", encoding="utf-8").read())
ilog = scrub(io.open(".b193_run437_ilog.txt", encoding="utf-8").read())
idx = None
...
io.open(".b193_run437_prev.txt", "w", encoding="utf-8").write("OK %d %d" % (len(ev), len(ilog)))
print("done")