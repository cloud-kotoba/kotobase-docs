#!/bin/bash
# falsify 228 K-Z3 23hr n-add run515A-C + landing control (independent continuation set)
...
for s in A B C; do
  run_series "search$s" "$SRCH" "$BASE/.f515_run515$s.ttfb"
done
run_series "landing" "$CTRL" "$BASE/.f515_landing.ttfb"
