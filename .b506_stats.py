#!/usr/bin/env python3
# K-Z3 stats for run506 (nearest-rank p50, cold>=0.5s), ASCII-safe output
import os

BASE = "/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/.run506_raw"
COLD = 0.5
...
res = []
res.append(stats(os.path.join(BASE, "runA", "resp.txt"), "run506A"))
res.append(stats(os.path.join(BASE, "runB", "resp.txt"), "run506B"))
res.append(stats(os.path.join(BASE, "runC", "resp.txt"), "run506C"))
res.append(stats(os.path.join(BASE, "control", "resp.txt"), "control(signup)"))

outtext = "\n".join(res)
with open(os.path.join(BASE, "..", ".b506_stats.txt"), "w") as f:
    f.write(outtext + "\n")
print(outtext)