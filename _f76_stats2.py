...
while i < len(src):
    ...
    if m:
        ...
        codes = []
        j = i + 1
        while j < len(src) and re.match(r'^\d{3} \d', src[j]):
            code, t = src[j].split()
            codes.append(code)
            vals.append(float(t))
            j += 1
        ...
        mx = vals[-1] * 1000 if vals else 0
        out_lines.append(f"{m.group(1)}: n={n} ok={codes.count('200')} cold(>=0.5s)={cold} p50={p50*1000:.0f}ms max={mx:.0f}ms")
        i = j
    else:
        i += 1
out = '\n'.join(out_lines)
open('/tmp/f76_stats2.txt', 'w').write(out + '\n')
print(out)
