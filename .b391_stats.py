#!/usr/bin/env python3
# stats for K-Z3 run391: per-file cold count (>=0.5s), p50 (nearest-rank), max.
import math, json
...
for k,fn in files.items():
    ...
    out[k]={"n":len(vals),"codes_200":codes.count("200"),"non200":[c for c in codes if c!="200"],
            ...
            "warm_p50":round(p50(warm),4) if warm else None,
            "max":round(max(vals),4) if vals else None,
            "cold_vals":[round(v,4) for v in cold]}
print(json.dumps(out,ensure_ascii=False,indent=1))
total_cold = out["391A"]["cold"]+out["391B"]["cold"]+out["391C"]["cold"]
