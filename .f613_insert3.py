...
lines = s.split("\n")
# rows: 281 (hypothesis-only, no evidence), 283 is preceded by a bench continuation? probe said 280 is K-Z2 evidence cont, 281 = K-Z3 row w/ NEXT tail... both 281 and 283 are K-Z3 rows.
# Evidence row = the one that has evidence text = the one whose NEXT-labeled tail means it is the *hypothesis* table row; but there are 2 K-Z3 rows.
# From probe: 281 END = "...NEXT: 委ねる ... secret 不含 (curl + python3 stats のみ)。" -> that row already has evidence text (rich). 283 also similar.
# Distinguish: 283 row is the one right after 282 continuation? Actually 282 is a continuation of 281? No—282 starts ' bench 2026-09-14' meaning 281 row wraps onto 282.
# Physical rows: 281 is the K-Z3 row WITH evidence (282 is its wrapped continuation? No—282 is a separate physical line starting with ' bench').
# In this table, evidence wraps are separate physical lines starting with space. The true K-Z3 evidence row = last physical line of that cell block.
# 283 is another K-Z3 row (duplicate table entry). Evidence should go to the row ending with '... secret 不含 (curl + python3 stats のみ)。' i.e. row 281 (0-based) per probe.
i = 281
