...
with io.open('.b451_ev.txt', encoding='utf-8') as f:
    ev = f.read()
ev = ev.strip()
...
with io.open('.b451_iter.txt', encoding='utf-8') as f:
    iter_txt = f.read()
...
for i, l in enumerate(out):
    if l.strip() == '## Iteration log':
        ...
        print('line after:', out[i + 1][:60])
        break
for i, l in enumerate(out):
    if l.startswith('| K-Z3 | worker |'):
        print('KZ3 row line', i + 1, 'tail80:', repr(l[-80:]))
        break
print('OK')