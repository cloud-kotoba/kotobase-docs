// falsify 第70回 — K-Z3 6時台 n 積み増し (run188A-C, bench 第69回 run187A-C 直後の追加 n)
...
const SEARCH = "https://search.kotobase.net/search?q=falsify70kz3";
...
const result = { purpose: "falsify 70 K-Z3 6時台 n 積み増し (run188A-C)", observedAt: new Date().toISOString(), runs: [] };
for (let r = 0; r < 3; r += 1) {
  const runs = [];
  for (let i = 0; i < 20; i += 1) runs.push(run(SEARCH));
  result.runs.push(summarize(runs, `run${["A", "B", "C"][r]} search`));
}
const control = [];
