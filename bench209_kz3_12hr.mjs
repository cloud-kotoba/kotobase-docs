// bench 第205回 — K-Z3 12時台帯初計測 run462A-C (rank 第205回 NEXT, 現時刻帯 12時台)
// 同測定法: /search?q= n=20 × 3 run + landing control n=20, 別接続 curl, production 実測 (gate 外)
// cold>=0.5s TTFB, nearest-rank p50, secret 不含。
...
const SEARCH = "https://search.kotobase.net/search?q=test";
const LANDING = "https://kotobase.net/signup";
...
const result = { purpose: "bench 第205回 K-Z3 12時台帯初計測 run462A-C", observedAt: new Date().toISOString(), runs: [] };
for (let r = 0; r < 3; r += 1) {
  const runs = [];
  for (let i = 0; i < 20; i += 1) runs.push(run(SEARCH));
  result.runs.push(summarize(runs, `run${String.fromCharCode(65 + r)} search`));
}
const control = [];
