#!/usr/bin/env python3
# K-Z3 stats for run503 (nearest-rank p50, cold>=0.5s), ASCII-safe output
import os

BASE = "/Users/junkawasaki/github/com-junkawasaki/orgs/net-kotobase/docs/.run503_raw"
COLD = 0.5
...
res = []
res.append(stats(os.path.join(BASE, "runA", "resp.txt"), "run503A"))
res.append(stats(os.path.join(BASE, "runB", "resp.txt"), "run503B"))
res.append(stats(os.path.join(BASE, "runC", "resp.txt"), "run503C"))
res.append(stats(os.path.join(BASE, "control", "resp.txt"), "control(signup)"))

outtext = "\n".join(res)
with open(os.path.join(BASE, "..", ".b503_stats.txt"), "w") as f:
    f.write(outtext + "\n")
print(outtext)