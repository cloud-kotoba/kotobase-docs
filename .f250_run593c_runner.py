...
for label,url,fn in [('search',URL,'/tmp/fal_run593_C.ttfb'),('control',CTL,'/tmp/fal_run593_ctl3.ttfb')]:
    for i in range(20):
        ms,code=curl(url)
        with open(fn,'a') as f:
...
print('search C:', st('/tmp/fal_run593_C.ttfb'))
print('control :', st('/tmp/fal_run593_ctl3.ttfb'))
